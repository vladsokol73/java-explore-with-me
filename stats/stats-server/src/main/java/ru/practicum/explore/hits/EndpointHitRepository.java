...
public interface EndpointHitRepository  extends JpaRepository<EndpointHit, Long> {
    ...
    @Query("SELECT new ru.practicum.EndpointHit.EndpointHitDtoResp(e.app, e.uri, COUNT(distinct e.ip)) " +
    ...
    @Query("SELECT new ru.practicum.EndpointHit.EndpointHitDtoResp(e.app, e.uri, COUNT(e.ip)) " +
            "FROM EndpointHit AS e " +
            "WHERE e.timestamp >= :start " +
            "AND e.timestamp <= :end " +
            "AND e.uri IN :uris " +
